...
void NexuizPlayerList_setPlayerList(entity me, string plist)
{
	float buf,i,n;
	string s;
...
string NexuizPlayerList_getPlayerList(entity me, float i, float key)
{
	return bufstr_get(me.playerList, i * PLAYERPARM_COUNT + key);
}

void NexuizPlayerList_resizeNotify(entity me, vector relOrigin, vector relSize, vector absOrigin, vector absSize)
{
	me.itemAbsSize = '0 0 0';
	SUPER(NexuizPlayerList).resizeNotify(me, relOrigin, relSize, absOrigin, absSize);
...
void NexuizPlayerList_drawListBoxItem(entity me, float i, vector absSize, float isSelected)
{
	string s;
	string score;
	float t;
