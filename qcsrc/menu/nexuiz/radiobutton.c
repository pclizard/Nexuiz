...
void NexuizRadioButton_configureNexuizRadioButton(entity me, float theGroup, string theCvar, string theValue, string theText)
{
	if(theCvar)
	{
		me.cvarName = theCvar;
...
void NexuizRadioButton_setChecked(entity me, float val)
{
	if(val != me.checked)
	{
		me.checked = val;
...
void NexuizRadioButton_loadCvars(entity me)
{
	if(me.cvarValue)
	{
		if(me.cvarName)
...
void NexuizRadioButton_draw(entity me)
{
	if not(me.cvarValue)
		if not(me.cvarName)
		{
	...
	SUPER(NexuizRadioButton).draw(me);
}
void NexuizRadioButton_saveCvars(entity me)
{
	if(me.cvarValue)
	{
		if(me.cvarName)
