...
void NexuizCampaignList_configureNexuizCampaignList(entity me)
{
...
void NexuizCampaignList_destroy(entity me)
{
	if(me.campaignGlob >= 0)
		search_end(me.campaignGlob);
}

void NexuizCampaignList_loadCvars(entity me)
{
	// read campaign cvars
	if(campaign_name)
...
void NexuizCampaignList_saveCvars(entity me)
{
...
void NexuizCampaignList_campaignGo(entity me, float step)
{
...
void NexuizCampaignList_draw(entity me)
{
	if(cvar(me.cvarName) != me.campaignIndex || cvar_string("g_campaign_name") != campaign_name)
		me.loadCvars(me);
	SUPER(NexuizCampaignList).draw(me);
}

void NexuizCampaignList_resizeNotify(entity me, vector relOrigin, vector relSize, vector absOrigin, vector absSize)
{
	me.itemAbsSize = '0 0 0';
	SUPER(NexuizCampaignList).resizeNotify(me, relOrigin, relSize, absOrigin, absSize);
...
void NexuizCampaignList_clickListBoxItem(entity me, float i, vector where)
{
...
void NexuizCampaignList_drawListBoxItem(entity me, float i, vector absSize, float isSelected)
{
	string s;
	float p;
	vector theColor;
...
void NexuizCampaignList_setSelected(entity me, float i)
{
	// prevent too late items from being played
	SUPER(NexuizCampaignList).setSelected(me, min(i, me.campaignIndex));
}

float NexuizCampaignList_keyDown(entity me, float scan, float ascii, float shift)
{
	if(scan == K_ENTER || scan == K_SPACE || scan == K_KP_ENTER)
		CampaignList_LoadMap(me, me);
	else
		return SUPER(NexuizCampaignList).keyDown(me, scan, ascii, shift);
	return 1;
}
#endif
