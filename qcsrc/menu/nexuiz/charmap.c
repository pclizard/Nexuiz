...
void NexuizCharmap_configureNexuizCharmap(entity me, entity theTextbox)
{
	me.controlledTextbox = theTextbox;
	me.configureImage(me, me.image);
}

float NexuizCharmap_mouseMove(entity me, vector coords)
{
	float x, y, c;
	x = floor(coords_x * 16);
	y = floor(coords_y * 10);
...
float NexuizCharmap_mouseDrag(entity me, vector coords)
{
	return me.mouseMove(me, coords);
}
float NexuizCharmap_mousePress(entity me, vector coords)
{
...
float NexuizCharmap_mouseRelease(entity me, vector coords)
{
	if(!me.pressed)
		return 0;
	me.mouseMove(me, coords);
...
float NexuizCharmap_keyDown(entity me, float key, float ascii, float shift)
{
	switch(key)
	{
		case K_LEFTARROW:
...
void NexuizCharmap_focusLeave(entity me)
{
	me.controlledTextbox.saveCvars(me.controlledTextbox);
}
void NexuizCharmap_draw(entity me)
{
	if(me.focused)
	{
