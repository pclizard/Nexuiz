#ifdef INTERFACE
CLASS(NexuizHUDVoteDialog) EXTENDS(NexuizRootDialog)
	METHOD(NexuizHUDVoteDialog, fill, void(entity))
	ATTRIB(NexuizHUDVoteDialog, title, string, "Vote Panel")
	ATTRIB(NexuizHUDVoteDialog, color, vector, SKINCOLOR_DIALOG_TEAMSELECT)
	ATTRIB(NexuizHUDVoteDialog, intendedWidth, float, 0.4)
	ATTRIB(NexuizHUDVoteDialog, rows, float, 15)
	ATTRIB(NexuizHUDVoteDialog, columns, float, 4)
	ATTRIB(NexuizHUDVoteDialog, name, string, "HUDvote")
ENDCLASS(NexuizHUDVoteDialog)
#endif

#ifdef IMPLEMENTATION
void NexuizHUDVoteDialog_fill(entity me)
{
	entity e;
	...
	me.TR(me);
		me.TD(me, 1, 3, e = makeNexuizCheckBox(0, "hud_panel_vote", "Enable panel"));
	me.TR(me);
	...
	me.TR(me);
		...
		me.TD(me, 1, 1.2, e = makeNexuizTextLabel(0, "Padding:"));
			me.TD(me, 1, 2.6, e = makeNexuizTextSlider(strzone(strcat("hud_panel_", panelname, "_bg_padding"))));
				...
				e.configureNexuizTextSliderValues(e);
	me.TR(me);
		me.TD(me, 1, 1.4, e = makeNexuizTextLabel(0, "Alpha after voting:"));
			me.TD(me, 1, 2.6, e = makeNexuizSlider(0.1, 1, 0.1, "hud_panel_vote_alreadyvoted_alpha"));
}
#endif
