...
void NexuizKeyBinder_configureNexuizKeyBinder(entity me)
{
...
void NexuizKeyBinder_resizeNotify(entity me, vector relOrigin, vector relSize, vector absOrigin, vector absSize)
{
	SUPER(NexuizKeyBinder).resizeNotify(me, relOrigin, relSize, absOrigin, absSize);
...
void NexuizKeyBinder_keyGrabbed(entity me, float key, float ascii)
{
	float n, j, k, nvalid;
	string func;
...
void NexuizKeyBinder_editUserbind(entity me, string theName, string theCommandPress, string theCommandRelease)
{
	string func, descr;
...
void NexuizKeyBinder_clickListBoxItem(entity me, float i, vector where)
{
...
void NexuizKeyBinder_setSelected(entity me, float i)
{
	...
	SUPER(NexuizKeyBinder).setSelected(me, i);
}
float NexuizKeyBinder_keyDown(entity me, float key, float ascii, float shift)
{
	float r;
	r = 1;
	switch(key)
	{
		...
		case K_BACKSPACE:
			...
			break;
		default:
			r = SUPER(NexuizKeyBinder).keyDown(me, key, ascii, shift);
			break;
	}
	return r;
}
void NexuizKeyBinder_drawListBoxItem(entity me, float i, vector absSize, float isSelected)
{
	string s;
	float j, k, n;
	vector theColor;
