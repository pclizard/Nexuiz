...
CLASS(NexuizCvarList) EXTENDS(NexuizListBox)
	...
	METHOD(NexuizCvarList, mouseRelease, float(entity, vector))
...
void NexuizCvarList_configureNexuizCvarList(entity me)
{
...
void NexuizCvarList_destroy(entity me)
{
	buf_del(me.handle);
}
void NexuizCvarList_setSelected(entity me, float i)
{
	string s;

	SUPER(NexuizCvarList).setSelected(me, i);
	if(me.nItems == 0)
		return;
...
void NexuizCvarList_resizeNotify(entity me, vector relOrigin, vector relSize, vector absOrigin, vector absSize)
{
	SUPER(NexuizCvarList).resizeNotify(me, relOrigin, relSize, absOrigin, absSize);
...
void NexuizCvarList_drawListBoxItem(entity me, float i, vector absSize, float isSelected)
{
	string k, v, d;
	float t;
...
float NexuizCvarList_keyDown(entity me, float scan, float ascii, float shift)
{
	...
	else if(SUPER(NexuizCvarList).keyDown(me, scan, ascii, shift))
		return 1;
...
float NexuizCvarList_mouseRelease(entity me, vector pos)
{
	if(me.pressed == 2)
		me.cvarValueBox.parent.setFocus(me.cvarValueBox.parent, me.cvarValueBox);
	return SUPER(NexuizCvarList).mouseRelease(me, pos);
}
