#ifdef INTERFACE
CLASS(NexuizHUDNotificationDialog) EXTENDS(NexuizRootDialog)
	METHOD(NexuizHUDNotificationDialog, fill, void(entity))
	ATTRIB(NexuizHUDNotificationDialog, title, string, "Notification Panel")
	ATTRIB(NexuizHUDNotificationDialog, color, vector, SKINCOLOR_DIALOG_TEAMSELECT)
	ATTRIB(NexuizHUDNotificationDialog, intendedWidth, float, 0.4)
	ATTRIB(NexuizHUDNotificationDialog, rows, float, 15)
	ATTRIB(NexuizHUDNotificationDialog, columns, float, 4)
	ATTRIB(NexuizHUDNotificationDialog, name, string, "HUDnotify")
ENDCLASS(NexuizHUDNotificationDialog)
#endif

#ifdef IMPLEMENTATION
void NexuizHUDNotificationDialog_fill(entity me)
{
	entity e;
	...
	me.TR(me);
		...
		me.TD(me, 1, 1.2, e = makeNexuizTextLabel(0, "Padding:"));
			me.TD(me, 1, 2.6, e = makeNexuizTextSlider(strzone(strcat("hud_panel_", panelname, "_bg_padding"))));
				...
				e.configureNexuizTextSliderValues(e);
	me.TR(me);
		me.TD(me, 1, 2, e = makeNexuizTextLabel(0, "Notifications:"));
	me.TR(me);
		me.TDempty(me, 0.2);
		me.TD(me, 1, 3, e = makeNexuizCheckBox(0, "hud_panel_notify_print", "Also print notifications to the console"));
	me.TR(me);
		me.TDempty(me, 0.2);
		me.TD(me, 1, 3, e = makeNexuizCheckBox(0, "hud_panel_notify_flip", "Flip notify order"));
	me.TR(me);
		me.TDempty(me, 0.2);
		me.TD(me, 1, 1.2, e = makeNexuizTextLabel(0, "Entry lifetime:"));
			me.TD(me, 1, 2.6, e = makeNexuizSlider(3, 15, 1, "hud_panel_notify_time"));
	me.TR(me);
		me.TDempty(me, 0.2);
		me.TD(me, 1, 1.2, e = makeNexuizTextLabel(0, "Entry fadetime:"));
			me.TD(me, 1, 2.6, e = makeNexuizSlider(0.5, 5, 0.5, "hud_panel_notify_fadetime"));
}
#endif
