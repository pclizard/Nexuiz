...
void NexuizMapList_configureNexuizMapList(entity me)
{
...
float NexuizMapList_g_maplistCacheQuery(entity me, float i)
{
	return stof(substring(me.g_maplistCache, i, 1));
}
void NexuizMapList_g_maplistCacheToggle(entity me, float i)
{
...
void NexuizMapList_draw(entity me)
{
	if(me.startButton)
		me.startButton.disabled = ((me.selectedItem < 0) || (me.selectedItem >= me.nItems));
	SUPER(NexuizMapList).draw(me);
}

void NexuizMapList_resizeNotify(entity me, vector relOrigin, vector relSize, vector absOrigin, vector absSize)
{
	me.itemAbsSize = '0 0 0';
	SUPER(NexuizMapList).resizeNotify(me, relOrigin, relSize, absOrigin, absSize);
...
void NexuizMapList_clickListBoxItem(entity me, float i, vector where)
{
...
void NexuizMapList_drawListBoxItem(entity me, float i, vector absSize, float isSelected)
{
...
void NexuizMapList_refilter(entity me)
{
	float i, j, n;
	string s;
	float gt, f;
...
void NexuizMapList_refilterCallback(entity me, entity cb)
{
	me.refilter(me);
}
...
float NexuizMapList_keyDown(entity me, float scan, float ascii, float shift)
{
	string ch, save;
	...
	else if(ascii >= 32 && ascii != 127)
	{
		...
		return SUPER(NexuizMapList).keyDown(me, scan, ascii, shift);
	return 1;
}
