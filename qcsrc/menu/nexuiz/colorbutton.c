...
void NexuizColorButton_configureNexuizColorButton(entity me, float theGroup, float theColor, float theValue)
{
...
void NexuizColorButton_setChecked(entity me, float val)
{
	if(val != me.checked)
	{
		me.checked = val;
...
void NexuizColorButton_loadCvars(entity me)
{
	if not(me.cvarName)
		return;
...
void NexuizColorButton_saveCvars(entity me)
{
	if not(me.cvarName)
		return;
...
void NexuizColorButton_draw(entity me)
{
	me.color2 = colormapPaletteColor(me.cvarValueFloat, me.cvarPart);
	SUPER(NexuizColorButton).draw(me);
}
#endif
