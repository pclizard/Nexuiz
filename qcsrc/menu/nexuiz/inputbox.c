...
void NexuizInputBox_configureNexuizInputBox(entity me, float doEditColorCodes, string theCvar)
{
...
void NexuizInputBox_focusLeave(entity me)
{
	me.saveCvars(me);
}
void NexuizInputBox_setText(entity me, string new)
{
	if(me.text != new)
	{
		SUPER(NexuizInputBox).setText(me, new);
		me.onChange(me, me.onChangeEntity);
	}
	else
		SUPER(NexuizInputBox).setText(me, new);
}
void NexuizInputBox_loadCvars(entity me)
{
	if not(me.cvarName)
		return;
	SUPER(NexuizInputBox).setText(me, cvar_string(me.cvarName));
}
void NexuizInputBox_saveCvars(entity me)
{
	if not(me.cvarName)
		return;
	cvar_set(me.cvarName, me.text);
}
float NexuizInputBox_keyDown(entity me, float key, float ascii, float shift)
{
	float r;
	r = 0;
	...
	if(SUPER(NexuizInputBox).keyDown(me, key, ascii, shift))
		r = 1;
	return r;
}
#endif
