...
void NexuizSkinList_configureNexuizSkinList(entity me)
{
...
void NexuizSkinList_loadCvars(entity me)
{
	string s;
	float i, n;
...
void NexuizSkinList_saveCvars(entity me)
{
	cvar_set("menu_skin", me.skinParameter(me, me.selectedItem, SKINPARM_NAME));
}

string NexuizSkinList_skinParameter(entity me, float i, float key)
{
	return bufstr_get(me.skinlist, i * SKINPARM_COUNT + key);
}

void NexuizSkinList_getSkins(entity me)
{
...
void NexuizSkinList_destroy(entity me)
{
	buf_del(me.skinlist);
}

void NexuizSkinList_resizeNotify(entity me, vector relOrigin, vector relSize, vector absOrigin, vector absSize)
{
	me.itemAbsSize = '0 0 0';
	SUPER(NexuizSkinList).resizeNotify(me, relOrigin, relSize, absOrigin, absSize);
...
void NexuizSkinList_drawListBoxItem(entity me, float i, vector absSize, float isSelected)
{
	string s;
...
void NexuizSkinList_setSkin(entity me)
{
	me.saveCvars(me);
...
void NexuizSkinList_clickListBoxItem(entity me, float i, vector where)
{
...
float NexuizSkinList_keyDown(entity me, float scan, float ascii, float shift)
{
	if(scan == K_ENTER || scan == K_KP_ENTER) {
		...
		return SUPER(NexuizSkinList).keyDown(me, scan, ascii, shift);
}
#endif
