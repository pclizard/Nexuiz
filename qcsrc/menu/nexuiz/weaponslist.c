...
void NexuizWeaponsList_configureNexuizWeaponsList(entity me)
{
	me.configureNexuizListBox(me);
}
void NexuizWeaponsList_draw(entity me)
{
	// read in cvar?
	string s, t;
	...
	SUPER(NexuizWeaponsList).draw(me);
}
...
void NexuizWeaponsList_resizeNotify(entity me, vector relOrigin, vector relSize, vector absOrigin, vector absSize)
{
	SUPER(NexuizWeaponsList).resizeNotify(me, relOrigin, relSize, absOrigin, absSize);

	me.realFontSize_y = me.fontSize / (absSize_y * me.itemHeight);
	me.realFontSize_x = me.fontSize / (absSize_x * (1 - me.controlWidth));
	me.realUpperMargin = 0.5 * (1 - me.realFontSize_y);
}
float NexuizWeaponsList_mouseDrag(entity me, vector pos)
{
	float f, i;
	i = me.selectedItem;
	f = SUPER(NexuizWeaponsList).mouseDrag(me, pos);
	if(me.selectedItem != i)
		cvar_set("cl_weaponpriority", swapInPriorityList(cvar_string("cl_weaponpriority"), me.selectedItem, i));
	return f;
}
string NexuizWeaponsList_toString(entity me)
{
	float n, i;
	string s;
	entity e;
...
void NexuizWeaponsList_drawListBoxItem(entity me, float i, vector absSize, float isSelected)
{
	entity e;
	if(isSelected)
...
float NexuizWeaponsList_keyDown(entity me, float scan, float ascii, float shift)
{
	if(ascii == 43) // +
	{
	...
	else if(SUPER(NexuizWeaponsList).keyDown(me, scan, ascii, shift))
		return 1;
	return 0;
}
#endif
