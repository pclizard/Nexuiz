...
CLASS(NexuizServerList) EXTENDS(NexuizListBox)
	...
	ATTRIB(NexuizServerList, iconsSizeFactor, float, 0.85)

	ATTRIB(NexuizServerList, realFontSize, vector, '0 0 0')
	ATTRIB(NexuizServerList, realUpperMargin, float, 0)
	ATTRIB(NexuizServerList, columnIconsOrigin, float, 0)
	ATTRIB(NexuizServerList, columnIconsSize, float, 0)
	...
	ATTRIB(NexuizServerList, seenIPv4, float, 0)
	ATTRIB(NexuizServerList, seenIPv6, float, 0)
...
void NexuizServerList_configureNexuizServerList(entity me)
{
...
void NexuizServerList_setSelected(entity me, float i)
{
	float save;
	save = me.selectedItem;
	SUPER(NexuizServerList).setSelected(me, i);
	/*
...
void NexuizServerList_refreshServerList(entity me, float mode)
{
...
void NexuizServerList_focusEnter(entity me)
{
...
void NexuizServerList_draw(entity me)
{
	float i, found, owned;
	...
	SUPER(NexuizServerList).draw(me);
}
...
void NexuizServerList_setSortOrder(entity me, float field, float direction)
{
...
void NexuizServerList_positionSortButton(entity me, entity btn, float theOrigin, float theSize, string theTitle, void(entity, entity) theFunc)
{
...
void NexuizServerList_resizeNotify(entity me, vector relOrigin, vector relSize, vector absOrigin, vector absSize)
{
	SUPER(NexuizServerList).resizeNotify(me, relOrigin, relSize, absOrigin, absSize);
...
void NexuizServerList_clickListBoxItem(entity me, float i, vector where)
{
...
void NexuizServerList_drawListBoxItem(entity me, float i, vector absSize, float isSelected)
{
...
float NexuizServerList_keyDown(entity me, float scan, float ascii, float shift)
{
	float i;
	vector org, sz;
	...
	else if(SUPER(NexuizServerList).keyDown(me, scan, ascii, shift))
		return 1;
